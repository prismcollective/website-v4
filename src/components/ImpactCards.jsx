...
export default function ImpactCards() {
  return (
    <section className="relative max-w-[1360px] mx-auto px-6 py-24 overflow-hidden">
      <div
        className="absolute inset-6 -z-10 border border-dashed border-black/30 bg-black/5"
        aria-label="illustration placeholder"
      >
        <span className="absolute inset-0 flex items-center justify-center text-xs font-medium uppercase tracking-[0.2em] text-black/50">
      ...
      <div className="flex flex-col md:flex-row gap-10 md:gap-6 items-start">
        {CARDS.map((card, i) => (
          <div
            key={card.title}
            className="bg-white border border-black flex flex-col gap-4 items-end p-4 w-full md:w-[350px]"
            style={{ marginTop: i === 1 ? "3rem" : i === 2 ? "6rem" : 0 }}
          >
            <div className="flex items-start justify-between w-full">
              <div className="border-[0.6px] border-black px-4 py-2">
                {/* Body 1 Heavy (24px / medium / -3% / 120%) at md+ */}
                <span className="text-black font-sans text-xl font-medium tracking-[-0.03em] md:text-body-1-heavy whitespace-nowrap">
                  {card.title}
                </span>
              </div>
            ...
            <p className="text-black font-sans text-lg font-medium leading-tight tracking-[-0.02em] md:text-body-2-heavy w-full text-left">
              {card.body}
            </p>
          </div>
        ))}
